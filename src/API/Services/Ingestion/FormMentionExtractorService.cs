    ...
    ILogger<FormMentionExtractorService> logger)
{
    ...
    public async Task ExtractPdfFormMentionsAsync(string sourceFile, IReadOnlyList<ParsedChunk> chunks, string agent, CancellationToken ct = default)
    {
    ...
    {
        return Path.GetFullPath(Path.Combine(environment.ContentRootPath, _options.DataPath, "form-registry-draft.json"));
    }
    ...
    private class FormRegistryDraftEntry
    {
        [JsonPropertyName("form_name")]
        public string FormName { get; set; } = string.Empty;
